...
// LAÇOS E REPETIÇÕES
		 - ENQUANTO A CONDIÇÃO IMPOSTA NA REPETIÇÃO FOR VERDADEIRA O LAÇO CONTINUA EXECUTANDO, QUANDO FOR FALSA ELE PARA.

		 while(){
			 É UMA ESTRUTURA DE REPETIÇÃO COM TESTE LÓGICO NO INCIO
			 OU SEJA, PRIMEIRO O TESTE É EXECUTADO, SE FOR VERDADEIRO O BLOCO É EXECUTADO
		 }

		 do {
			É UMA ESTRUTURA DE REPETIÇÃO COM TESTE LÓGICO NO FINAL
			OU SEJA, PRIMEIRO O BLOCO É EXECUTADO E DEPOIS O TESTE É REALIZADO, SE FOR VERDADEIRO EXECUTA NOVAMENTE
...
// VARIÁVEIS COMPOSTAS - [ARRAY]
		   ...
		   - VARIAVEIS COMPOSTAS DEVEM SER CAPAZES DE ARMAZENAR VÁRIOS VALORES EM UMA MESMA ESTRUTURA

		   ARRAY / VETOR
		   UM ARRAY É UMA VARIAVEL QUE POSSUI VARIOS ELEMENTOS, CADA ELEMENTO É COMPOSTO POR SEU VALOR E PELO SEU INDICE OU CHAVE DE IDENTIFICACAO
		   SAO AS VARIAVEIS COMPOSTAS: CONSTITUIDAS POR ELEMENTOS, VALOR DO CONTEUDO E INDICES 

		   EXEMPLO: 

		   let num = [5, 8, 4]
		   
		   PARA ACRESCENTAR MAIS UMA "CASA" OU VALOR AO ARRAY EXECUTE O COMANDO ABAIXO

		   num[3] = 6 - É COMO SE O MESMO TIVESSE SIDO CRIADO DA SEGUINTE FORMA 
		   let num = [5, 8, 4, 6]

		   PODEMOS UTILIZAR O COMANDO ABAIXO PARA ACRESCENTAR O VALOR AUTOMATICAMENTE NA ULTIMA CASA

		   num.push(7) - FICA DA SEGUINTE FORMA
		   let num = [5, 8, 4, 6, 7]

		   COMANDO PARA DESCOBRIR O TAMANHO DO ARRAY, QUANTOS VALORES OU CASAS POSSUI
		   num.length
		   console.log(num.length)

		   COMANDO PARA ORDENAR EM ORDEM CRESCENTE O ARRAY
		   num.sort()
...
//FUNÇÕES
	FUNÇÕES SAO ACOES EXECUTADAS ASSIM QUE SÃO CHAMADAS OU EM DECORRÊNCIA DE UM EVENTO
	UMA FUNCAO PODE RECEBER PARAMETROS E RETORNAR UM RESULTADO 

		   - TODA FUNÇÃO NECESSITA DE UMA CHAMADA
		   ...
		   function acao(parametros){
			...
			acao (5) // FAZ A CHAMADA INSERINDO O NUMERO 5 COMO PARAMETRO, ASSIM EXECUTANDO A FUNCAO

			EXEMPLO VERIFICANDO SE UM NÚMERO É PAR OU IMPAR 

			function parimp(n) { //(N) É O PARAMETRO
				if(n%2==0) {	//O IF REPRESENTA TODA A ACAO A SER EXECUTADA
					return 'par' // RETORNO OU RESULTADO
				} else {
					return 'impar' //RETORNO OU RESULTADO
				}
			}
